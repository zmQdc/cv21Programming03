...
if __name__ == "__main__":
    ...
    # 获取model
    model = pspnet(num_classes, inputs_size, downsample_factor=downsample_factor, backbone=backbone,
    ...
    if True:
        ...
        if eager:
            gen = tf.data.Dataset.from_generator(
                partial(Generator(Batch_size, train_lines, inputs_size, num_classes, aux_branch, dataset_path).generate,
                        random_data=True), (tf.float32, tf.float32))
            gen_val = tf.data.Dataset.from_generator(
                partial(Generator(Batch_size, val_lines, inputs_size, num_classes, aux_branch, dataset_path).generate,
                        random_data=False), (tf.float32, tf.float32))
            ...
            gen_val = Generator(Batch_size, val_lines, inputs_size, num_classes, aux_branch, dataset_path).generate(
                False)
